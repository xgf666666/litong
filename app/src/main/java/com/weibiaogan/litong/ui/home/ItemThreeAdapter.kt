...
class ItemThreeAdapter(datas : List<HomeBean.ProjectBean>?) : BaseQuickAdapter<HomeBean.ProjectBean, BaseViewHolder>(R.layout.home_item_three_view,datas){
    ...
    override fun convert(helper: BaseViewHolder?, item: HomeBean.ProjectBean?) {
        ...
        helper?.setText(R.id.tv_home_item_three_time,item?.end_time)
        helper?.setText(R.id.tv_home_item_three_distance,item?.distance.toString())
        helper?.getView<ImageView>(R.id.iv_home_item_three_img)?.loadImag("",plach = R.mipmap.img_face)
    }
